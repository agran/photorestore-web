...
export default function Editor() {
  ...
  const { currentImageUrl, originalImageUrl, setImage } = useEditorStore();
  ...
  return (
    <div className="container py-6">
      <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-[220px_1fr_200px]">
        {/* Tools column */}
        <aside>
          <ToolPanel />
        </aside>
          ...
          <Card className="h-full">
            ...
            <CardContent className="flex flex-col gap-2 overflow-y-auto">
              ...
              {history.map((entry) => (
                <button
                  key={entry.id}
                  className="w-full overflow-hidden rounded border text-left transition-colors hover:border-primary"

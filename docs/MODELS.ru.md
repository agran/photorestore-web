...
| `scrfd-10g`               | SCRFD-10G-KPS            | Скрытие лиц    | Apache-2.0  | 8.2 МБ | 1×3×640×640  | WebGPU ✅ (NCHW, FP16 weights) |
...
- **SCRFD-10G** ранее откатывался на WASM из-за того же бага JSEP, но теперь работает на WebGPU через workaround `preferNchw: true` + `graphOptimizationLevel: 'basic'` (тот же фикс, что включает ESRGAN-модели). Веса модели конвертированы в FP16 (вход/выход остаётся FP32), размер уменьшен вдвое (~8 МБ против ~16 МБ), скорость инференса выше на ~12%. Точность проверена: IoU ≥ 0.998 относительно FP32-бейзлайна.

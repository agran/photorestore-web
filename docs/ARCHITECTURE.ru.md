...
Детекция лиц на фото использует два прохода для максимальной полноты:

1. **Глобальный проход** (letterbox всей картинки) — `fitCanvasLetterbox()` уменьшает
   фотографию до размера модели (640×640) с сохранением пропорций. Ловит крупные
   лица (портреты), которые занимают несколько тайлов и не вписываются ни в один
   тайл 640×640, ни в якорный масштаб детектора.

2. **Тайловый проход** — стандартная `splitDetectionTiles()` с overlap 64px. Ловит
   средние и маленькие лица на групповых фото.

Результаты обоих проходов объединяются и пропускаются через NMS-дедупликацию.
Глобальный проход пропускается только для изображений ≤640×640 (они полностью
помещаются в один тайл и без масштабирования).

### Clamped-линейное масштабирование эффектов

Функции `scaleKernel()` и `scaleEffectStrength()` в `anonymizeEffects.ts` —
общие для фото- и видео-пайплайнов (раньше были продублированы в `anonymizeVideo.ts`).

- **`scaleKernel(userValue, bboxWidth)`** — линейное масштабирование для padding/feather:
  `userValue × (bboxWidth / 100)`. Калибровано на лицо шириной 100px.

- **`scaleEffectStrength(userValue, bboxWidth, minValue)`** — clamped-линейное для силы
  эффекта (blur radius, pixelate block size): `max(1, bboxWidth / 100)`. Лицо ≥100px
  получает линейное масштабирование, лицо <100px сохраняет значение слайдера (factor ≥1).
  Результат: примерно постоянное количество блоков пикселизации на лицо любого размера
  ≥100px. Маленькие лица получают пиксели размером со слайдер — выглядят как пикселизация,
  а не шум. Заменяет ранее использовавшуюся суперлинейную формулу `(faceWidth/100)^1.3`.

- **Симметричный expandBox**: когда лицо близко к краю холста, padding обрезается
  симметрично по обеим сторонам оси (раньше обрезалась только левая/верхняя сторона,
  что приводило к смещению расширенного бокса вправо/вниз — маски визуально сдвигались).

PreviewCanvas теперь применяет те же `scaleKernel`/`scaleEffectStrength`, что и Apply —
превью точно отражает результат. По умолчанию `pixelateSize = 10` (было 16).

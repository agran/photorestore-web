...
### Суперлинейное масштабирование эффектов
...
- **`scaleEffectStrength(userValue, bboxWidth, minValue)`** — суперлинейное для силы
  эффекта (blur radius, pixelate block size): `(bboxWidth / 100) ^ 1.3`. Большие лица
  получают непропорционально сильнее размытие/пикселизацию. face=200px → ×2.46 (не ×2).

...
- [~] v0.7.4: Mask review after first pass + detection data reuse (keyframe replay, faster re-runs). UI — video scrubber~~, keyframe editor, preview during processing~~
- [x] v0.7.x hardening (post-review): streaming tiling (P0 OOM fix), worker crash-recovery, blob-URL revoke audit (editor + video + Dropzone + PreviewCanvas), vectorized hot loops for tensor↔canvas conversion, pre-allocated detect canvas, blob-URL thumbnails instead of synchronous toDataURL, MediaRecorder timeslice, removed double NMS, unified file validation, discriminated union in pipelineRunner, toxic TOAST_REMOVE_DELAY ~16 min → 5 s, dead-code cleanup (see [CODE_REVIEW.md](./CODE_REVIEW.md))
